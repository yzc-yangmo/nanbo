...
def init_spider(express_code):
    if express_code == 1:
        ...
        driver = undetected_chromedriver.Chrome(options=options, executable_path=r"C:\Users\24035\.cache\selenium"
                                                                                 r"\chromedriver\win64\114.0.5735.90"
                                                                                 r"\chromedriver.exe")
        rangle = (1210, 810, 1365, 890)  # 验证码固定位置
    elif express_code == 2:
        driver = undetected_chromedriver.Chrome(executable_path=r"C:\Users\24035\.cache\selenium"
                                                                                 r"\chromedriver\win64\114.0.5735.90"
                                                                                 r"\chromedriver.exe")
            ...
            for td in arg_driver.find_elements(By.XPATH, '//tbody//tr//td'):
                if td.get_attribute('class').find('disabled') != -1:
                    ...
                    if exist_by_id(arg_driver, 'nc_1_n1z'):
                        ...
                        print("不存在滑块！")
                    if exist_by_xpath(arg_driver, '//*[text()="预约失败"]'):
                        print("预约失败，正在重试....")
                    else:
                        print('提交成功！！')
                        print(now_time + '抢票成功')
